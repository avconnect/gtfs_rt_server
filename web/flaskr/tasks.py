...
def update_feeds():
    with scheduler.app.app_context():
        time_recorded = datetime.utcnow().replace(microsecond=0)
        feeds = db.session.query(Feed).all()
        for feed in feeds:
            print(f'Updating Feed {feed.id}')
            update_vehicle_position(feed.id, time_recorded)
            update_trip_updates(feed.id, time_recorded)
        print(f'All feeds updated\n')
